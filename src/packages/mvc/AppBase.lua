...
function AppBase:ctor(configs)
    self.configs_ = {
        ...
        defaultSceneName ="Scenes/LoadingScene/GameMain",
    }

    local director = cc.Director:getInstance()
	local eglView = director:getOpenGLView()
	if eglView then
		--eglView:setFrameSize(1136, 640)
		--director:setOpenGLView(eglView)
	end
	local screenSize = eglView:getFrameSize()
	local designSize = cc.size(1136, 640)
	local resourceSize = cc.size(1136, 640)

	if screenSize.height > resourceSize.height then
		eglView:setDesignResolutionSize(designSize.width, designSize.height, cc.ResolutionPolicy.FIXED_WIDTH)
		--director:setContentScaleFactor( math.min(resourceSize.height / designSize.height, resourceSize.width / designSize.width))
	else
		eglView:setDesignResolutionSize(designSize.width, designSize.height, cc.ResolutionPolicy.FIXED_WIDTH)
	end
